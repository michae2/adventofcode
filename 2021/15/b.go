...
func caveRisk(risks *[100][100]byte, x, y int) int {
	risk := int(risks[y%100][x%100] - '0')
	tile := x/100 + y/100
	return (risk+tile-1)%9 + 1
}

// Use A* search.
func main() {
	var risks [100][100]byte
	for y := 0; y < 100; y++ {
		var line []byte
		fmt.Scan(&line)
		copy(risks[y][:], line)
	...
	for current.point != end {
		for _, neighbor := range current.neighbors() {
			...
			risk := caveRisk(&risks, neighbor.x, neighbor.y) + current.risk
